...
namespace TarkovMonitor
{
    public partial class MainBlazorUI : Form
    {
        ...
        private const string MapFrameInjection =
            ...
            + "  var to = document.querySelector('.leaflet-bottom.leaflet-left');"
            + "  if (!to) { return; }"
            + "  ['.leaflet-top.leaflet-right', '.leaflet-bottom.leaflet-right'].forEach(function (selector) {"
            + "   var from = document.querySelector(selector);"
            ...
            + "  if (!relocateTimer) { relocateTimer = setTimeout(function () { relocateTimer = null; relocateControls(); }, 100); }"
